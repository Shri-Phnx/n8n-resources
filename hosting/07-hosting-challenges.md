...
### Challenge 1: Container cannot connect to Ollama or other local services
...
### Challenge 3: Credentials cannot decrypt after moving n8n
...
### Challenge 5: n8n uses too much memory or CPU
...
**Recommended VPS providers:** DigitalOcean, Hetzner (cheapest EU), AWS EC2, Azure, Contabo
...
### Challenge 4: VPS reboots and n8n does not restart
...
# Settings → Executions → Enable automatic cleanup
# Set EXECUTIONS_DATA_MAX_AGE=72 (hours) in .env
```
...
### Challenge 2: n8n cannot reach internal company APIs from cloud VPS
...
| Schedule does not fire on VPS reboot | Ensure `restart: unless-stopped` in Docker or `pm2 startup` for npm |
...
| Challenge | Fix |
|-----------|-----|
| OAuth redirect URL does not match | OAuth apps require an exact redirect URL. Set it to `https://your-n8n.com/rest/oauth2-credential/callback` |
| Token expires and workflow breaks | Enable automatic token refresh in credential settings. Some OAuth2 providers need a refresh token scope |
...
| **Store credentials securely** | n8n encrypts credentials with `N8N_ENCRYPTION_KEY` — back it up, do not share it |
| **Restrict webhook exposure** | Use Header Auth on all webhooks |
| **Limit execution history** | Enable pruning — do not store sensitive data in execution logs indefinitely |
| **Use environment variables** | Never hardcode API keys in workflow expressions — use n8n Variables or credentials |
| **Regular backups** | Daily backup of the n8n data volume (see `09-backup-recovery-updates.md`) |
